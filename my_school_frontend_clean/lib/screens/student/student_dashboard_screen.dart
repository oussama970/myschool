...
class _StudentDashboardScreenState extends State<StudentDashboardScreen> {
  ...
  void _initializePages() {
    _pages = [
      ...
      StudentEventsScreen(
        ...
        onEventsChanged: _loadPendingEvents, // ✅ AJOUT: callback pour mettre à jour
      ),
    ...
    setState(() {
      _pages = [
        ...
        StudentEventsScreen(
          ...
          studentName: widget.studentName,
          studentClass: _studentClass,
          onEventsChanged: _loadPendingEvents,
        ),
        StudentScheduleScreen(studentClass: _studentClass),
        StudentMessagesScreen(studentId: _studentId, studentName: widget.studentName, studentClass: _studentClass),
        StudentProfileScreen(studentEmail: widget.email, studentName: widget.studentName, studentClass: _studentClass),
  ...
  // ✅ CORRIGÉ: Méthode pour charger les événements en attente
  Future<void> _loadPendingEvents() async {
    ...
    try {
      ...
      if (result['success'] && mounted) {
        ...
        // - réponse = 'pending' 
        // - ET date limite non dépassée (si une date limite existe)
        final pendingCount = events.where((event) {
          ...
          if (responseDeadline != null && responseDeadline.toString().isNotEmpty) {
            try {
              ...
              if (deadline.isBefore(now)) return false;
            } catch (_) {}
          }
          return true;
        }).length;
        ...
        print('📊 Événements en attente: $_pendingEvents');
      }
    } catch (e) {
