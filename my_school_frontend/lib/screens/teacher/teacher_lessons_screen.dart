...
class _TeacherLessonsScreenState extends State<TeacherLessonsScreen> {
  ...
  List<Map<String, dynamic>> _attachedFiles = []; // [{name, path, type}]
  ...
  void _loadMockData() {
    setState(() {
      ...
      _reminders = [
        {'id': '7', 'title': 'Réunion parents', 'description': '18h salle 101', 'date': '20/03/2024'},
        {'id': '8', 'title': 'Sortie scolaire', 'description': 'Prévoir pique-nique', 'date': '05/04/2024'},
      ];
      _isLoading = false;
    });
  }
    ...
    try {
      ...
      if (photo != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('📸 Photo ajoutée: ${photo.name}'), backgroundColor: Colors.green, duration: const Duration(seconds: 1)),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _pickImageFromGallery() async {
    try {
      ...
      if (image != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('🖼️ Image ajoutée: ${image.name}'), backgroundColor: Colors.green, duration: const Duration(seconds: 1)),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _pickFile() async {
    try {
      ...
      if (result != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('📎 ${result.files.length} fichier(s) ajouté(s)'), backgroundColor: Colors.green, duration: const Duration(seconds: 1)),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _saveContent() async {
    if (_titleController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Veuillez entrer un titre'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    try {
      ...
      if (_isEditing && _editingId != null && _editingId!.isNotEmpty) {
        if (_editingType == 'Rappel') {
          ...
          result = await ApiService.updateLesson(
            ...
            description: _descriptionController.text,
            type: _selectedType,
            deadline: _showDeadline ? _deadlineController.text : null,
            files: fileNames,
          );
        }
      } else {
      ...
      if (result['success']) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_isEditing ? '✅ Contenu modifié avec succès' : '✅ Contenu ajouté avec succès'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['message'] ?? 'Erreur lors de l\'opération'),
          ...
          TextButton(
            onPressed: () async {
              ...
              if (result['success']) {
                await _loadData();
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('🗑️ Contenu supprimé'), backgroundColor: Colors.red),
                );
              } else {
                ScaffoldMessenger.of(context).showSnackBar(
