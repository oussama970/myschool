...
const getDashboardStats = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      stats: { totalTeachers, totalParents, totalStudents, totalClasses, pendingTeachers, classes, parentsStats: { oneChild, twoChildren, threePlusChildren } }
    });
  } catch (error) {
...
const getRecentActivities = async (req, res) => {
  try {
    ...
    for (const teacher of recentTeachers) {
      activities.push({ id: teacher._id, type: 'teacher', title: 'Nouvel enseignant', description: teacher.fullName, email: teacher.email, time: teacher.createdAt, icon: 'person_add', color: '#0288D1' });
    }
    for (const classItem of recentClasses) {
      activities.push({ id: classItem._id, type: 'class', title: 'Nouvelle classe', description: classItem.name, time: classItem.createdAt, icon: 'add_box', color: '#4CAF9F' });
    }
    for (const parent of recentParents) {
      activities.push({ id: parent._id, type: 'parent', title: 'Nouveau parent', description: parent.fullName, email: parent.email, time: parent.createdAt, icon: 'family_restroom', color: '#FF9800' });
    }
    for (const student of recentStudents) {
      activities.push({ id: student._id, type: 'student', title: 'Nouvel élève', description: student.fullName, className: student.className || 'Non assigné', time: student.createdAt, icon: 'school', color: '#9C27B0' });
    }
    
    activities.sort((a, b) => new Date(b.time) - new Date(a.time));
    
    res.json({ success: true, activities: activities.slice(0, 10), count: activities.length, totalMessages: 0 });
  } catch (error) {
    console.error('Erreur:', error);
    res.status(500).json({ success: false, message: 'Erreur serveur' });
  }
